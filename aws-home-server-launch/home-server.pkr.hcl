...
locals {
    timestamp_id = "SNAPSHOT" #replace(replace(timestamp(),":","_"),"T","_")
}
...
build {
	...
	source "source.amazon-ebs.debian_server" {
	    name                  = "debian"
		ami_name              = "debian_arm_${local.timestamp_id}"
	...
	provisioner "shell" {
		...
		inline = [
                ...
                "sudo apt-get install gcc -y", # both matrix and mastidon
               ...
               # "sudo apt-get install python3-acme python3-certbot python3-mock python3-openssl python3-pkg-resources python3-pyparsing python3-zope.interface -y",
		        ...
		        ######          ensure we have the right time on this server
